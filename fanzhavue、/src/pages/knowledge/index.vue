		...
		<view class="header-section">
			<view class="header-left">
				<view class="logo-badge">
					<image class="logo-badge-img" src="/static/反诈知识库 .png" mode="aspectFit" />
				</view>
		...
		<view class="stats-card">
			<view class="stats-header">
				<view class="stats-title">
					<uni-icons type="piechart" size="18" :color="accentIconColor"></uni-icons>
					分类统计
				</view>
		...
		<view class="knowledge-list">
			<view class="list-header">
				<view class="list-title">
					<uni-icons type="book" size="18" :color="accentIconColor"></uni-icons>
					知识条目
				</view>
			</view>
			...
			<view v-else>
				...
				>
					<view class="item-icon" :style="{ background: getCategoryColor(item.category) }">
						<image class="item-icon-img" :src="getCategoryIconSrc(item.category)" mode="aspectFit" />
					</view>
...
export default {
	data() {
		return {
			searchQuery: '',//搜索框值(索引值)
			selectedCategory: '全部',//分类筛选值
			selectedSource: '全部',//来源筛选值
	...
	computed: {
		accentIconColor() {
			return this.isGuardianTheme ? '#b71c1c' : '#2f64f5'
		},
		filteredItems() {
			let items = this.list
	...
	methods: {
		loadKnowledge() {
			...
			this.error = null
			uni.request({
				url: `${getApiBaseUrl()}/api/knowledge`,
				method: 'GET',
				success: (res) => {
					this.loading = false
		...
		async updateKnowledgeBase() {
			...
			try {
				const res = await uni.request({
					url: `${getApiBaseUrl()}/api/knowledge/update`,
					method: 'POST',
					header: {
		...
		//没分类的兜底
		getCategoryIconSrc(category) {
			if (!category) return '/static/典型案例库分类.png'
			const fileMap = {
				'黑产技术': '黑客技术分类.png'
			}
			const file = fileMap[category] || `${category}分类.png`
			return `/static/${file}`
		},
...
<style lang="scss">
	...
	.add-card {
		background: linear-gradient(135deg, #5d8fff, #356af2);
	}
	...
	/* 监护人主题：知识库页内硬编码蓝色改为与全局 theme-guardian 一致 */
	.page.theme-guardian {
	...
	.theme-guardian .logo-badge {
		background: linear-gradient(145deg, #ef5350, #c62828) !important;
		box-shadow: 0 14rpx 32rpx rgba(198, 40, 40, 0.35) !important;
	}

	.theme-guardian .search-wrap {
		background: #fff5f5;
	...
	.theme-guardian .action-btn {
		...
		box-shadow: 0 8rpx 20rpx rgba(183, 28, 28, 0.35) !important;
	}

	.theme-guardian .sync-card {
		background: linear-gradient(135deg, #e53935 0%, #b71c1c 100%) !important;
	}

	.theme-guardian .add-card {
		background: linear-gradient(135deg, #ef5350, #b71c1c 100%) !important;
	}

	.theme-guardian .stats-count {
	...
	.theme-guardian .retry-btn,
	.theme-guardian .reset-btn {
		background: linear-gradient(135deg, #ef5350, #c62828) !important;
	}

	.theme-guardian .picker-confirm {
		color: #c62828 !important;
	}

	.theme-guardian .dialog-btn.submit {
		background: linear-gradient(135deg, #ef5350, #c62828) !important;
		box-shadow: 0 8rpx 20rpx rgba(183, 28, 28, 0.3) !important;
	}
